...
app.put("/update_data_siswa", (req, res) => {
  ...
  var query_update =
    "UPDATE tb_siswa SET nama = ?, umur = ?, alamat = ? WHERE nis = ?";
  var values_update = [nama, umur, alamat, nis];

  mydb.query(query_update, values_update, function (err, result, fields) {
    if (err) throw err;
    console.log(result);

    var response_payload = {
      description: "Berhasil memperbarui data siswa",
      mysql_response: result,
    };
...
app.delete("/delete_data_siswa", (req, res) => {
  ...
  var query_delete = "DELETE FROM tb_siswa WHERE nis = ?";

  mydb.query(query_delete, nis, function (err, result, fields) {
    if (err) throw err;
